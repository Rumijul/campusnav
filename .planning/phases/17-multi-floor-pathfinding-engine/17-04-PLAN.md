...
must_haves:
  truths:
    ...
    - "flattenNavGraph is no longer imported anywhere in the codebase"
...
<task type="auto">
  ...
  <action>
    ...
    **2. FloorPlanCanvas.tsx — build floorMap and pass to calls:**
    ...
    After changes: run `grep -r "flattenNavGraph" src/` — output must be empty (no remaining imports of the removed function).
  </action>
  <verify>
...
```
grep -r "flattenNavGraph" src/
```
Must return no output — flattenNavGraph is fully removed and unused.
</verification>
...
- flattenNavGraph does not appear anywhere in src/
- npm test: all pass
