...
plan: "01"
subsystem: testing
tags: [vitest, konva, gesture, touch, tdd]

# Dependency graph
requires: []
provides:
  - "8 failing unit tests (RED) for gesture math helpers: toStageLocalFromScreen, computePivotPosition, applyRotationThreshold"
  - "Test contract defining expected behavior for TOUCH-01, TOUCH-02, TOUCH-03"
affects:
  - 21-02  # GREEN phase — must implement exports to make these tests pass

# Tech tracking
tech-stack:
  added: []
  patterns:
    - "TDD RED: test imports named exports that don't yet exist to create hard failures"
    - "MockStage factory uses real Konva.Transform for accurate absolute transform simulation"
    - "Pure formula tests avoid React DOM rendering by testing extracted helpers directly"
...
key-decisions:
  - "Tests import named exports (toStageLocalFromScreen, computePivotPosition, applyRotationThreshold) that Plan 02 must add to useMapViewport.ts"
  - "MockStage getAbsoluteTransform built from real Konva.Transform (translate + rotate + scale) — matches internal Konva behavior exactly"
  - "TOUCH-01 Test 2 verifies stageLocal at 45° equals 100*sqrt(2) ≈ 141.42 (x) and 0 (y) — proves inverse transform handles rotation correctly"
  - "TOUCH-01 Test 3 verifies stageLocal at 90° is (200, -150) — proves axes swap correctly under 90° rotation"
  - "TOUCH-03 threshold is strict greater-than (> 2°), not greater-or-equal — Test 7 explicitly validates the boundary"

patterns-established:
  - "Named export pattern: extract pure math helpers from hooks as named exports for testability without React test env"
